 ...
 * those addresses need to be mapped to the registers defined for I2C TPMs in
...
static int tpm_tis_i2c_read16(struct tpm_tis_data *data, u32 addr,
			      u16 *result)
{
	__le16 result_le;
	int rc;
...
static int tpm_tis_i2c_read32(struct tpm_tis_data *data, u32 addr,
			      u32 *result)
{
	__le32 result_le;
	int rc;
...
static int tpm_tis_i2c_write32(struct tpm_tis_data *data, u32 addr,
			       u32 value)
{
	__le32 value_le;
	int rc;
...
static int tpm_tis_i2c_reset(struct i2c_client *dev)
{
	struct gpio_desc *reset_gpio;
	int ret;

	reset_gpio = devm_gpiod_get_optional(&dev->dev, "reset",
					     GPIOD_OUT_HIGH);
	if (IS_ERR(reset_gpio)) {
		ret = PTR_ERR(reset_gpio);
		if (ret != -EPROBE_DEFER)
			dev_err(&dev->dev, "failed to get reset GPIO: %d\n",
				ret);
		return ret;
	}
	...
	/*
	 * The DT marks reset as GPIO_ACTIVE_LOW. GPIO descriptor values are
	 * logical, so 1 asserts reset and 0 deasserts it.
	 */
			     ...
			     const struct i2c_device_id *id)
{
	...
	dev_info(&dev->dev, "probing TPM at I2C address 0x%02x\n",
		 dev->addr);

	ret = tpm_tis_i2c_reset(dev);
	if (ret)
		return ret;
	...
	if (ret) {
		dev_err(&dev->dev,
			"failed to read TPM I2C interface capability: %d\n",
			ret);
		return ret;
	}

	ret = tpm_tis_i2c_write_bytes(&phy->priv, TPM_LOC_SEL,
				      sizeof(locality), &locality);
	if (ret) {
		dev_err(&dev->dev, "failed to select locality: %d\n", ret);
		return ret;
	}

	ret = tpm_tis_core_init(&dev->dev, &phy->priv, -1, &tpm_i2c_phy_ops,
				NULL);
	if (ret)
		dev_err(&dev->dev, "failed to initialize TPM: %d\n", ret);
